...
while opcion != "1" and opcion != "2" and opcion != "3" and opcion != "4":
        ...
        while opcABM != "1" and opcABM != "2" and opcABM != "3" and opcABM != "4":
            ...
            if opcABM == "1": # Alta nueva película
                ...
                while addOtra == "s":
                    ...
                    # ID
                    if len(pelis_dict["peliculas"]) == 0:
                        pelicula["ID"] = 1
                    else:
                        ultimo_id = pelis_dict["peliculas"][len(pelis_dict["peliculas"])-1]["ID"]
                    ...
                    while not duracion.isnumeric():
                        duracion = input(" Ingrese la duración en minutos de la película(En numeros) > ")
                    else:
                        pelicula["duracion"] = int(duracion)
                    #Sinopsis (string)
                    while pelicula["sinopsis"] == "":
                        pelicula["sinopsis"] = input(" Ingrese una sinapsis de la pelicula > ")
                ...
                while opcEliminar != "1" and opcEliminar != "2" and opcEliminar != "3":
                    ...
                    if opcEliminar == "1": #buscar por id
                        id = int(input("\n CINEMA+\n Eliminar una película existente\n\n Ingrese el numero de identificación (Id)\n > "))
                        for x, y in buscarID(id).items():
                            print(x.ljust(13, " "),": ", y)
                        eliminar_id = input(" Desea eliminar esta pelicula? [S/N] > ").lower()
                        if eliminar_id == "s":
                            pelicula_eliminada = buscarID(id)
                            eliminar = buscarID(id)
                            pelis_dict["peliculas"].pop(eliminar)
                    elif opcEliminar == "2":
    ...
    # Salir        
        ...
        if salir == "s":
            pelis_dict["peliculas"] += peliculas
            
            with open("peliculas.json", "w") as archivo:
                json.dump(pelis_dict, archivo, indent=4)
            os.system("exit")
            os.system("cls")
        else:
            opcion = ""
