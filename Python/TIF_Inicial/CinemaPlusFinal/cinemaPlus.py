...
opc_menu_principal = ("ABM películas", "Calificación de películas", "Reportes y estadísticas", "Salir")
...
def mostrar_pelicula(dicc_pelicula_mostrar):
    for x, y in dicc_pelicula_mostrar.items():
        if x == "disponible" and y == True:
            y = "Disponible"
        elif x == "disponible" and y == False:
            y = "No Disponible"
...
def imprimir_menu_principal():
    while True:
        ...
        while not opcion.isnumeric() or int(opcion) not in range(1, len(opc_menu_principal)+1):
            opcion = input(f"¡Opción no válida! Ingrese nuevamente una opción[1-{len(opc_menu_principal)}]> ")
        if opcion == "1":
            imprimir_menu_abm() 
        elif opcion == "2":
            calificar_pelicula()
...
def imprimir_menu_abm():
    while True:
        ...
        while not opcion.isnumeric() or int(opcion) not in range(1, len(opc_menu_abm)+1):
            opcion = input(f"¡Opción no válida! Ingrese nuevamente una opción[1-{len(opc_menu_abm)}]\n> ")
        if opcion == "1":
            cargar_pelicula()
...
def calificar_pelicula():
    ...
    for pelicula in range(0, 10):
        ...
        while calificar != "s" and calificar != "n":
            calificar = input("¡Ingreso una opción no válida! Debe ser \"S\" para si o \"N\" para no\n¿Desea calificar la pelicula? [S/N]\n> ").lower()
        if calificar == "s":
            calificacion = input("Califique la pelicula con un número [1-10]\n> ")
            ...
            if lista_peliculas[indice]["calificacion"] == 0.0:
...
def imprimir_menu_reportes():
    while True:
        ...
        while not opcion.isnumeric() or int(opcion) not in range(1, len(opc_menu_reporte)+1):
            opcion = input(f"¡Opción no válida! Ingrese nuevamente una opción[1-{len(opc_menu_reporte)}]\n> ")
        if opcion == "1":
...
def imprimir_menu_buscar():
    while True:
        ...
        while not opcion.isnumeric() or int(opcion) not in range(1, len(opc_menu_buscar)+1):
            opcion = input(f"¡Opción no válida! Ingrese nuevamente una opción[1-{len(opc_menu_buscar)}]\n> ")
        if opcion == "1":
            indice = buscar_id() 
            return indice
        elif opcion == "2":
...
def buscar_id():
    while True:
        ...
        while True:
            busqueda = input("Ingrese el ID de la película\n> ")
        ...
        for pelicula in range(0, len(lista_peliculas)): 
            ...
            while repetir_busqueda != "s" and repetir_busqueda != "n":
                repetir_busqueda = input("¡Ingreso una opción no válida! Debe ser \"S\" para si o \"N\" para no\n¿Desea realizar otra búsqueda? [S/N]\> ").lower()
...
def buscar_titulo():
    ...
    while buscar:
        ...
        while True:
            busqueda = input("Ingrese el título de la película\> ")
            if busqueda == "":
                print("¡Debe ingresar un título o palabra a buscar!")
            else:
                break
        ...
        if len(indices_buscar_pelicula) == 0:
            ...
            opcion = input("Si encontró la película buscada ingrese el número de opción, de lo contrario ingrese \"N\"\n> ")
            while True:
                if opcion.isnumeric() and int(opcion) in range(1, len(indices_buscar_pelicula) + 1):
                    ...
                    opcion = input("¡Opción ingresada no válida!\n Si encontró la película buscada ingrese el número de opción, de lo contrario ingrese \"N\"\n> ")

def eliminar_pelicula(indice):
    limpiar_pantalla()
    print("CINEMA+\nEliminar película existente\n")
    mostrar_pelicula(lista_peliculas[indice])
    confirmar = input(f"\n¿Confirma que desea eliminar la película {lista_peliculas[indice]['titulo']}? [S/N]\n> ").lower()
...
def cargar_pelicula():
    ...
    while agregar:
        ...
        while True:
            guardar_alta = input("¿Desea agregar la película? [S/N]\n> ")
            if guardar_alta.lower() == "s":
                 lista_peliculas.append(dicc_pelicula)
                 id_max = dicc_pelicula["id"]
                 guardar("peliculas.json", lista_peliculas)
                 guardar("max_id.json", id_max) 
...
def modificar_pelicula(indice_lista_pelicula):
    ...
    for x, y in lista_peliculas[indice_lista_pelicula].items():
        ...
        while modificar != "n" and modificar != "s":
            modificar = input("¡Opción ingresada no válida! Debe ingresar \"S\" por si o \"N\" por no\n> ").lower()
...
def agregar_generos(dicc_pelicula):
    ...
    while True:
        ...
        if not opcion_genero.isnumeric():
            ...
            print(f"¡Opcion no válida! Debe ingresar un numero 1 al {len(lista_generos)}")

        elif lista_generos[int(opcion_genero) - 1] in generos:
            print("¡El género ingresado ya está cargado!")
        else:
...
def agregar_duracion(dicc_pelicula):
    ...
    duracion = input("> ")
    while not duracion.isnumeric() or int(duracion) < 0 or int(duracion) > 999:
    ...
    while True:
        ...
        if not opcion_clasificacion.isnumeric() :
          ...
          if int(opcion_clasificacion) not in range(1, len(lista_clasificacion) + 1):
             print(f"¡Opcion no válida! Debe ingresar un numero 1 al {len(lista_clasificacion)}")
          else:
...
def listar_peliculas_top(lista_peliculas):
    ...
    if len(peliculas_ordenadas) >= 15:
        rango = range(0, 15)
    else:
        rango = range(0, len(peliculas_ordenadas) - 1)
    w = "TOP"
    x = "TÍTULO"
    y = "GÉNERO"
    z = "CALIFICACIÓN"
    print(f"╔{'═' * 3}╦{'═' * 100}╦{'═' * 40}╦{'═' * 12}╗\n║{w}║{x.center(100, ' ')}║{y.center(40, ' ')}║{z}║\n╠{'═' * 3}╬{'═' * 100}╬{'═' * 40}╬{'═' * 12}╣")
    for pelicula in rango:
